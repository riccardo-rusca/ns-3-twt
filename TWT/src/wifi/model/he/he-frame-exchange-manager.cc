                                     ...
                                     const WifiTxVector& txVector, bool inAmpdu)
{
 ...
 // Add additional packet tags to gather some extra PHY and MAC layer metrics
 RxSignalInfoTag rxinfotag;
 rxinfotag.SetSnr(rxSignalInfo.snr);
 rxinfotag.SetRSSI(rxSignalInfo.rssi);
 rxinfotag.SetSnrdB(rxSignalInfo.snr_db);
 if(!mpdu->GetPacket()->PeekPacketTag(rxinfotag))
 {
      mpdu->GetPacket()->AddPacketTag (rxinfotag);
      NS_LOG_DEBUG("Add RX Signal Info Tag with SNR value :" << rxinfotag.GetSnr());
 }
  ...
  if(txVector.IsMu ())
    {
        ...
        {
            // This line is for getting the STA ID of the first associated STA
            // uint16_t staId = txVector.GetHeMuUserInfoMap().begin()->first;

            // This line is for getting the STA ID of the current associated STA
            uint16_t staId = m_staMac->GetAssociationId ();

            mcstag.SetMcs(txVector.GetMode (staId).GetMcsValue ());

            if(!mpdu->GetPacket()->PeekPacketTag(mcstag))
            {
                mpdu->GetPacket()->AddPacketTag (mcstag);
                NS_LOG_DEBUG("[DL] Add MCS Tag with MCS value :" << (int) mcstag.GetMcs ());
            }
        }
    } else if(m_apMac!=nullptr) {
      if(txVector.GetMode().GetModulationClass ()==WIFI_MOD_CLASS_HE || txVector.GetMode().GetModulationClass ()==WIFI_MOD_CLASS_HT) {
            mcstag.SetMcs(txVector.GetMode ().GetMcsValue ());
            if(!mpdu->GetPacket()->PeekPacketTag(mcstag))
            {
                mpdu->GetPacket()->AddPacketTag (mcstag);
                NS_LOG_DEBUG("[UL] Add MCS Tag with MCS value :" << (int) mcstag.GetMcs ());
            }
        }
  }
